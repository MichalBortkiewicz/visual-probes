...
class ConceptDiscovery(object):
    ...
    def load_concept_imgs(self, concept, max_imgs=1000):
        ...
        img_paths = [
            os.path.join(concept_dir, d) for d in tf.gfile.ListDirectory(concept_dir)
        ]
    ...
    def _extract_patch(self, image, mask):
        ...
        image_resized = (
            np.array(image.resize(self.image_shape, Image.BICUBIC)).astype(float) / 255
        )
    ...
    def _patch_activations(self, imgs, bottleneck, bs=100, channel_mean=None):
        ...
        if self.num_workers:
            pool = multiprocessing.Pool(self.num_workers)
            output = pool.map(
                lambda i: self.model.run_examples(
                    imgs[i * bs : (i + 1) * bs], bottleneck
                ),
        ...
        for bn in self.bottlenecks:
            ...
            for i in range(bn_dic["label"].max() + 1):
                ...
                if len(label_idxs) > self.min_imgs:
                    ...
                    concept_percentile_radius = np.percentile(
                        concept_costs, param_dicts[bn].pop("radius_percentile", 95)
                    )
                    ...
                    if highly_common_concept or cond2 or cond3:
                        ...
                        concept = "{}_concept{}".format(
                            self.target_class, concept_number
                        )
                        ...
                        bn_dic[concept + "_radius"] = concept_percentile_radius
            bn_dic.pop("label", None)
            bn_dic.pop("cost", None)
            self.dic[bn] = bn_dic
            pickle.dump(bn_dic, open(f"{self.target_class}_dict.pkl", "wb"))
    ...
    def _random_concept_activations(self, bottleneck, random_concept):
        ...
        rnd_acts_path = os.path.join(
            self.activation_dir, "acts_{}_{}".format(random_concept, bottleneck)
        )
    ...
    def load_cav_direction(self, c, r, bn, directory=None):
        ...
        if directory is None:
            directory = self.cav_dir
        params = tf.contrib.training.HParams(model_type="linear", alpha=0.01)
    ...
    def tcavs(self, test=False, sort=True, tcav_score_images=None):
        ...
        for bn in self.bottlenecks:
            for concept in self.dic[bn]["concepts"] + [self.random_concept]:
                ...
                if self.num_workers:
                    pool = multiprocessing.Pool(self.num_workers)
                    tcav_scores[bn][concept] = pool.map(
                        lambda rnd: t_func(rnd), randoms
                    )
                else:
